...
The suite covers historical schema-v1 adoption and rollback, fresh schema-v2 service rendering, a FreedOS-shaped `0.1.1` to `0.2.0` upgrade, desired-version and validator-contract migration, field-scoped rollback that preserves later organization edits, deterministic Hindsight identity, retry after an interrupted atomic write, idempotent apply, tampered preimage refusal, narrow rollback, rollback refusal after postimage drift, and scans for common personal-data and secret patterns.
