...
GitHub may briefly hide a newly created draft from release listings. The workflow makes one creation attempt, then performs a bounded series of read-only discovery checks. It proceeds only after exactly one matching release appears. If the draft remains invisible, publication fails without another creation attempt or any release mutation. The resolver lives inside the active workflow step, so replaying an older tag does not depend on a helper in that tag's checkout. Replaying the workflow remains safe.
