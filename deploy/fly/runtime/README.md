...
This directory defines the current 0.3.x Fly profile: the public control plane,
the public authenticated API, the private executive worker, and private
Hindsight derived memory. Replace every placeholder before deployment. Release
automation must deploy OCI digests, not mutable tags.

The approved target keeps one neutral canonical Vorton origin and adds a shared
supervised module runtime and worker pool. Independently deployable modules do
not receive one always-on Fly service by default. The current profile does not
implement the independently upgradable module runtime and is not deployment
...
Hindsight is derived and untrusted. A recall result is evidence input at most.
It cannot create Policy, grants, approvals, Work, receipts, or outcomes. Its Fly
service has no public port. API key tenant authentication remains mandatory on
the private network, and MCP is disabled. Each memory request and bank identity
also carries the exact Vorton installation, workspace, and realm.

Factory remains read-only in this current runtime. That is the first FreedOS
milestone, not the final architecture. The current AubOS Factory lineage
becomes Vorton's first-party Factory module, activated first in FreedOS and
later in any approved workspace. Governed execution follows through Vorton
module authority. There is no permanent external Factory mechanism.
...
The worker secret must match between API and worker. `VORTON_WORKER_MODEL` on
the API must exactly match `VORTON_CODEX_MODEL` on the worker. The API resolves
the installation, workspace, Work, worker, assigned role, evidence, and active
grant from authoritative Postgres before dispatch. It validates provider
response boundaries and records the worker run after return. The worker cannot
select an identity and has no database credential.

The worker runs one serialized Codex stream against a dedicated persistent
`auth.json`, disables every Codex tool surface during Council deliberation, and
rejects background work. One authentication cache may process explicitly
admitted confidential material from multiple workspaces only through a new
ephemeral provider session for each contribution. Prompt construction,
classification, egress Policy, budgets, logs, and receipts remain workspace
scoped. The cache contains authentication, not cross-workspace conversation
state. Do not share it with another service or machine.
