...
- Worker: `AUBOS_WORKER_SHARED_SECRET`, plus one-time `AUBOS_CODEX_AUTH_JSON` bootstrap material when the persistent cache is empty.
...
1. Stage `AUBOS_CODEX_AUTH_JSON` through standard input only while the auth volume is empty.
...
4. With explicit credential-removal authority, run `fly secrets unset AUBOS_CODEX_AUTH_JSON --app <worker-app>`.
...
Run the executable gate from a trusted host that can reach the private Hindsight service, such as through a temporary `fly proxy`. From an extracted release contract archive, set `AUBOS_HINDSIGHT_URL` and `AUBOS_HINDSIGHT_API_KEY`, then run `node bin/hindsight-canary.cjs`. A full source checkout may use `npm run runtime:hindsight-canary`. The archive command is bundled and runs without installing workspace dependencies. The canary uses only unique synthetic material, polls consolidation for at most five minutes by default, proves complete citation hydration and exhaustive source retirement, and deletes its synthetic bank even when a check fails. `AUBOS_HINDSIGHT_CANARY_TIMEOUT_MS` and `AUBOS_HINDSIGHT_CANARY_POLL_INTERVAL_MS` provide bounded operator overrides.
