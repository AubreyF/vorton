...
web_root="${VORTON_WEB_ROOT:-/usr/share/nginx/html}"
nginx_template="${VORTON_NGINX_TEMPLATE:-/etc/nginx/vorton.conf.template}"
nginx_output="${VORTON_NGINX_OUTPUT:-/etc/nginx/conf.d/default.conf}"

fail() {
  printf '%s\n' "Vorton web startup refused: $1" >&2
  exit 1
}
  ...
  case "$key" in
    ...
    sb_publishable_[A-Za-z0-9_-]*)
      printf '%s' "$key" | grep -Eq '^sb_publishable_[A-Za-z0-9_-]+$' || \
        fail "VORTON_PUBLIC_SUPABASE_ANON_KEY has an invalid publishable-key format"
      ;;
    eyJ[A-Za-z0-9._-]*)
      printf '%s' "$key" | grep -Eq '^eyJ[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$' || \
        fail "VORTON_PUBLIC_SUPABASE_ANON_KEY has an invalid legacy JWT format"
      payload="$(printf '%s' "$key" | cut -d. -f2 | tr '_-' '/+')"
      case $((${#payload} % 4)) in
        0) ;;
        2) payload="${payload}==" ;;
        3) payload="${payload}=" ;;
        *) fail "VORTON_PUBLIC_SUPABASE_ANON_KEY has an invalid legacy JWT payload" ;;
      esac
      decoded_payload="$(printf '%s' "$payload" | base64 -d 2>/dev/null)" || \
        fail "VORTON_PUBLIC_SUPABASE_ANON_KEY has an unreadable legacy JWT payload"
      printf '%s' "$decoded_payload" | grep -Eq '"role"[[:space:]]*:[[:space:]]*"anon"' || \
        fail "VORTON_PUBLIC_SUPABASE_ANON_KEY legacy JWT must have the anon role"
      ;;
    *) fail "VORTON_PUBLIC_SUPABASE_ANON_KEY must be a Supabase publishable key or legacy anon JWT" ;;
  esac
}
